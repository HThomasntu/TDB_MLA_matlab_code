...
[G S_R] = audioread('E:\Additional brood videos\20.08.21 sample 2_500hz.wav', [1 2]);
[G S_R] = audioread('E:\Additional brood videos\20.08.21 sample 2_500hz.wav', [1 round((((1*60)*60)+37*60)*S_R)]);
...
mite_bg_tdft(:,:,periodicity_section3) = two_D_FT_Gaussian(A(LL3:UL3,1),MF,tr,S_R,size(A,1)/(2*S_R));
...
for periodicity_section8 = 1:length(weak_bee)
    
LL8 = round((weak_bee(periodicity_section8)-window_length)*S_R);
UL8 = round((weak_bee(periodicity_section8)+window_length)*S_R);

weak_bee_tdft(:,:,periodicity_section8) = two_D_FT_Gaussian(G(LL8:UL8,1),MF,tr,S_R,size(G,1)/(2*S_R));
...
for uu8 = 1:size(weak_bee_tdft,3)
    for vv8 = 1:size(weak_bee_tdft,1)
        weak_bee_tdft_int(vv8,:,uu8) = interp1(1:size(weak_bee_tdft,2),weak_bee_tdft(vv8,:,uu8),1:5:size(weak_bee_tdft,2));
    end
%     subplot(2,1,1)
%     imagesc(log10(bee_bg_tdft(:,:,uu4)))
%     subplot(2,1,2)
...
cr_weak_bee_tdft = weak_bee_tdft_int(3:60,:,:);
...
for counter = 1:size(cr_weak_bee_tdft,3)
    weak_bee_mag = max(cr_weak_bee_tdft(:,1,counter)); 
    
    new_matrix8(:,:,counter) = weak_bee_mag; 
end 

scaled_weak_bee = cr_weak_bee_tdft./new_matrix8;
...
save('weak_bee_TDB.mat','scaled_bg','scaled_mite_final','scaled_bee_final','scaled_weak_bee','scaled_bee','S_R')
